...
function EventResultsPage() {
  ...
  const results = React.useMemo(() => {
    ...
    const eventResults = event.registeredUsers.map(userId => {
      ...
      const baseScore = Object.values(userSubmissions).reduce((acc, sub) => acc + sub.score, 0);
      
      const allProblemsSolved = event.problems.every(p => userSubmissions[p.problemId]);
      const totalScore = baseScore + (allProblemsSolved ? event.completionBonus : 0);

      return {
        user: user || { id: userId, name: 'Unknown User', email: '', avatar: '', points: 0, problemsSolved: 0, role: 'user' },
        score: totalScore,
        problemsSolved: Object.keys(userSubmissions).length,
      };
    });

    return eventResults.sort((a, b) => b.score - a.score).map((res, index) => ({ ...res, rank: index + 1 }));
  }, [event, users]);
  ...
  if (!event) {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 text-center">
        ...
        <Link to="/events" className="mt-4 inline-block bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-md">
          Back to Events
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        <Link to="/events" className="flex items-center text-orange-500 hover:text-orange-600 mb-4 font-medium">
          <ArrowLeft className="h-4 w-4 mr-2" />
          Back to All Events
        </Link>
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Results: {event.title}</h1>
        <p className="text-gray-600 dark:text-gray-400 mt-2">Final standings for the event.</p>
      </div>
      ...
      {results.length >= 3 && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          {[results[1], results[0], results[2]].map((result, index) => (
            <div key={result.rank} className={`rounded-lg border p-6 ${getRankBackground(result.rank)} ${result.rank === 1 ? 'md:scale-110 z-10' : ''}`}>
              <div className="text-center">
                <div className="flex justify-center mb-4">
                  {getRankIcon(result.rank)}
                </div>
                {result.user.avatar ? (
                  <img src={result.user.avatar} alt={result.user.name} className="w-16 h-16 rounded-full mx-auto mb-4 object-cover" />
                ) : (
                  <div className="w-16 h-16 rounded-full bg-orange-500 flex items-center justify-center mx-auto mb-4">
                    <span className="text-white font-bold text-xl">{getInitials(result.user.name)}</span>
                  </div>
                )}
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">{result.user.name}</h3>
                <p className="text-2xl font-bold text-orange-600 dark:text-orange-400 mt-2">{result.score.toLocaleString()}</p>
          ...
          <table className="w-full">
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {results.map(({ user, score, problemsSolved, rank }) => (
                <tr key={rank} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="text-sm font-bold text-orange-600 dark:text-orange-400">{score.toLocaleString()}</span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="text-sm text-gray-900 dark:text-white">{problemsSolved} / {event.problems.length}</span>
                  </td>
                </tr>
              ))}
            </tbody>
